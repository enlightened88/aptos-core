...
pub fn get_all_blocks_for_window(
    ...
    // But, this should be fine, as we'd fallback to state sync anyway.
...
mod tests {
    ...
    fn test_get_all_blocks_for_window_higher_rounds() {
        ...
        // Fetch all blocks for a window size of 2, and ensure it returns round 2 and 3
        ...
        // Fetch all blocks for a window size of 3, and ensure it returns round 1, 2, and 3
        ...
        // Fetch all blocks for a window size of 4, and ensure it returns round 0, 1, 2, and 3
        let ordered_block_round_zero = pending_blocks[0].clone();
        ...
        // Fetch all blocks for a window size of 5, and ensure it returns round 0, 1, 2, and 3
        let all_ordered_blocks =
            get_all_blocks_for_window(pending_block_store.clone(), &ordered_block_round_three, 5);
        assert_eq!(all_ordered_blocks.unwrap(), first_four_ordered_blocks);

        // Fetch all blocks for a window size of 10, and ensure it returns round 0, 1, 2, and 3
        let all_ordered_blocks =
    ...
    fn test_get_all_blocks_for_window_missing_blocks() {
        ...
        // Fetch all blocks for a window size of 3, and ensure it returns None (round 3 is missing)
        let all_ordered_blocks =
    ...
    fn test_get_all_blocks_for_window_missing_zero() {
        ...
        // Fetch all blocks for a window size of 6, and ensure it returns None (round 0 is missing)
        let all_ordered_blocks =
            get_all_blocks_for_window(pending_block_store.clone(), &ordered_block_round_five, 6);
        assert!(all_ordered_blocks.is_none());

        // Fetch all blocks for a window size of 10, and ensure it returns None (round 0 is missing)
        let all_ordered_blocks =
    ...
    fn create_and_add_pending_blocks(
        ...
        for i in 0..num_pending_blocks {
            ...
            let parent_block_info = if i == 0 {
                BlockInfo::random(999) // Random block parent for the first block
            } else {
